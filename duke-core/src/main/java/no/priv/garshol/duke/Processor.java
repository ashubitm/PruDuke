    ...
    while (it.hasNext()) {
      ...
      try {
        ...
        while (it2.hasNext()) {
          ...
          System.out.println("Record value is :"+record.toString()+"\n"+record.getProperties());;
...
/*    if(counter==0) {
    	counter++;
    Collection<Record> oldRecords =database1.getRecords();
  ...
  //  batchReady(records.size());
    //TODO if record has not been seen before load index - Only for the first time 
    batchReady(records.size());
    ...
    System.out.println("Time spent in indexing:"+indexing);
   ...
   * have been seen before.
   */
  public void deduplicate(Record record) {
    logger.info("Deduplicating batch of " + 1 + " records");
    if(counter==0) {
    	counter++;
    	if(database1!=null) {
    Collection<Record> oldRecords =database1.getRecords();
 ...
 //SIngle Record     	
  ...
  private void threadedmatch(Record record, boolean matchall) {
	    // split batch into n smaller batches
	    MatchThread[] threads = new MatchThread[this.threads];
	    for (int ix = 0; ix < threads.length; ix++)
	      threads[ix] = new MatchThread(ix, 1 / threads.length,
	                                    matchall);
	    int ix = 0;
	 //   for (Record record : records)
	      threads[ix++ % threads.length].addRecord(record);
  ...
  private void linkBatch(int dbno, Collection<Record> batch, boolean matchall) {
    ...
    // TODO matches for all record in Destination 
    //Re-write to control the batch 
    //Introduce a variable N where it will only try to match for N
    for (Record r : batch)
